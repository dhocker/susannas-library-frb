...
Susanna's Library FRB (Flask, React, Bootstrap)
----------------------------------------------

This web app is designed to manage an instance of the Susanna's Library FRB app.

Files of Note
-------------

requirements.txt - pip requirements file. Can be used to create the required virtual environment.
    The venv must be named susannas_library_frb.
    Note that the uwsgi file is set up for a flask virtual environment.
susannas_nginx_site - nginx configuration file for the app. This file should go in /etc/nginx/sites-available.
...
susannas_uwsgi_app.ini - uwsgi configuration file for the app. For non-Emperor mode,
